	...
	public F.Promise<SimpleResult> call(Context ctx) throws Throwable {
		...
		try{
	        ...
	        try{
	        	...
	        	return F.Promise.pure((SimpleResult) unauthorized("User " + Http.Context.current().args.get("username") + " is not authorized to access"));
	        }catch(ShuttingDownDBException sde){
	        	String message = sde.getMessage();
	        	Logger.info(message);
	        	return F.Promise.pure((SimpleResult) status(503, message));
	        }
			...
			result = F.Promise.pure((SimpleResult)forbidden(e.getMessage()));
		}catch (InvalidAppCodeException e){
			if (Logger.isDebugEnabled()) Logger.debug("ConnectToDB: Invalid App Code " + e.getMessage());
			result = F.Promise.pure((SimpleResult) unauthorized(e.getMessage()));
		}catch (Throwable e){
